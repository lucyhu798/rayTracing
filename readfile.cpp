                ...
                // Note that no transforms/stacks are applied to the colors.
                    ...
                    if (validinput) {
                        lightposn[4 * numused] = values[0];
                                    lightposn[1 + (4 * numused)] = values[1];
                                    lightposn[2 + (4 * numused)] = values[2];
                        lightposn[3 + (4 * numused)] = (float)0.0;
                        lightcolor[4 * numused] = values[3];
                                    lightcolor[1 + (4 * numused)] = values[4];
                                    lightcolor[2 + (4 * numused)] = values[5];
                        lightcolor[3 + (4 * numused)] = (float)1.0;
                       ++numused;
                    }
                } else if (cmd == "point") {
                    validinput = readvals(s,6,values);
                    if (validinput) {
                        lightposn[4 * numused] = values[0];
                                    lightposn[1 + (4 * numused)] = values[1];
                                    lightposn[2 + (4 * numused)] = values[2];
                        lightposn[3 + (4 * numused)] = (float)1.0;
                        lightcolor[4 * numused] = values[3];
                                    lightcolor[1 + (4 * numused)] = values[4];
                                    lightcolor[2 + (4 * numused)] = values[5];
                        lightcolor[3 + (4 * numused)] = (float)1.0;
                    ...
                    validinput = readvals(s, 3, values);
                    vertexpoint[vertexs]= vec3(values[0],values[1],values[2]);
                    ++vertexs;
                }
                ...
                // Also look at demo.txt to get a sense of why things are done this way.
                    ...
                    if (numobjects == maxobjects) { // No more objects
                        ...
                        if (cmd == "sphere") {
                            ...
                            if (validinput) {
                                obj->type = sphere;
                                (obj->centerSphere)[0] = values[0];
                                (obj->centerSphere)[1] = values[1];
                                (obj->centerSphere)[2] = values[2];
                                obj->size = values[3];
                            }
                        } else if (cmd == "tri") {
                            validinput = readvals(s, 3, values);
                            if (validinput) {
                                obj->type = triangle;
                                (obj->point)[0] = values[0];
                                (obj->point)[1] = values[1];
                                (obj->point)[2] = values[2];
                            }
                        }
                        ++numobjects;
                    }
                    ...
                    if (validinput) {
                        ...
                        // You might want to use helper functions on top of file.
                        // Also keep in mind what order your matrix is!
                        mat4 scal = Transform::scale(values[0], values[1], values[2]);
                        rightmultiply(scal, transfstack);
                    ...
                    if (validinput) {
                        ...
                        mat3 rot = Transform::rotate(values[3], axis);
                          mat4 rotationmatrix = mat4(rot[0][0], rot[0][1], rot[0][2], 0,
